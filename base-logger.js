...
import {
    CookieOverrides
} from './cookie-overrides/cookie-overrides';
import {
    APP_LOAD_ERROR_FLOW
...
const getInteractionLogName = (interactionName, eventGuid) =>
    eventGuid ? `[guid: ${eventGuid}] ${interactionName}` : interactionName;
const getInteractionNameFromLog = (interactionLogName) =>
    interactionLogName.replace(/\[guid:\s[^\]]+\]\s/, '');

const ARRAY_TYPE = '[object Array]';
const isArray = (possiblyArray) =>
    Object.prototype.toString.call(possiblyArray) === ARRAY_TYPE;
...
export default class BaseLogger {
    constructor(appName, appVersion, reporter, params) {
        ...
        this.enableSampleRateForAppNames =
            paramsWithDefaults.enableSampleRateForAppNames;
        ...
        this.sampleRateManager = new SampleRateManager(
            this.enableSampleRateForAppNames,
        );
        ...
        if (
            ...
            typeof this._constructorParamsOverrides.is_rollout === 'undefined'
        ) {
    ...
    _report(dataSource, reporterEndpoint, params) {
        const data = dataSource.mergeItems();
        if (
    ...
    reportNetworkAnalysis() {}

    _getAppLoadingPhaseMarksForApp({
        appId
    }) {
    ...
    appLoaded({
        ...
        const dataSource = this.dataSource
            ...
            .addItem(
                this.dataItems
                .duration(this._times.getAppLoadTime({
                    appId
                }))
    ...
    appLoadingPhaseStart(name, {
        ...
        const dataSource = this.dataSource
            ...
            .addItem(this.dataItems.appContext({
                appId,
                widgetId
            }))
    ...
    _sendPreviousPhaseIfNeeded(name, {
        appId,
        widgetId,
        paramsOverrides
    } = {}) {
        ...
        if (
            ...
            const loadingPhaseDataItem = this.dataItems.loadingPhaseFinish(
                previousPhase,
            );
    ...
    appLoadingPhaseFinish(
        ...
        const loadingPhaseBiItem = this.dataItems.biLoadPhaseFinish();
        const defaultParamsOverridesDataItem = this._getDataItemWithDefaultParamsOverrides({
            appName: appId
        }, );
        const overridesDataItem = this.dataItems.dataItem(paramsOverrides);
    ...
    _getDataItemWithDefaultParamsOverrides({
        appName = null
    } = {}) {
    ...
    interactionStarted(
        ...
        const interactionLogName = getInteractionLogName(
            interactionName,
            eventGuid,
        );
        ...
        if (stack) {
            this._outgoingInteractions[interactionLogName] =
                this._outgoingInteractions[interactionLogName] || [];
    ...
    interactionEnded(
        ...
        const outgoingInteraction = isArray(
                this._outgoingInteractions[interactionLogName],
            ) ?
            this._outgoingInteractions[interactionLogName].pop() :
            this._outgoingInteractions[interactionLogName];
        ...
        if (!isArray(this._outgoingInteractions[interactionLogName]) ||
            !this._outgoingInteractions[interactionLogName].length
        ) {
    ...
    _handleBlackboxPerformance() {
        ...
        if (actualEnv.addEventListener) {
            actualEnv.addEventListener(BLACKBOX, ({
                    detail
                }) =>
                        ...
                        case 'responsiveness':
                            if (performanceEntry.eventName && actualEnv.addEventListener) {
                                actualEnv.addEventListener(
                                    ...
                                    ({
                                        detail
                                    }) => this._sendBlackboxMeasurement(detail),
                                );
                            }
    ...
    _errorHappenedInAppLoadingPhasesFlow({
        appId
    } = {
        appId: this.appName
    }) {
        const appPhasesMarks = this._getAppLoadingPhaseMarksForApp({
            appId
        });
    ...
    _getInteractionNamesWhereErrorHappened() {
        const interactionsFromInstance = this._getInteractionNamesWhereErrorHappenedFromInstance();
    ...
    _getInteractionNamesWhereErrorHappenedFromInstance() {
        return Object.keys(this._outgoingInteractions).map((logName) =>
            getInteractionNameFromLog(logName),
        );
    }
        ...
        Promise.all([
            ...
            platformTenantAppNames.forEach((appName) => {
                const dataSource = this.dataSource
                    ...
                    .addItem(this.dataItems.appName({
                        appName
                    }))
                    .addItem({
                        cls,
